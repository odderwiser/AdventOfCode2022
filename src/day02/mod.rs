...
fn part_2(filename: &str) -> u32 {
    iterate(filename, |x, y| {
        y.get_outcome_value() + Game::get_move_to_play(x, y).get_value()
    })
}

fn part_1(filename: &str) -> u32 {
    iterate(filename, |x, y| y.get_value() + Game::get_win(x, y))
}

fn iterate(filename: &str, calculate_values: impl Fn(&Game, &Game) -> u32) -> u32 {
    parse_input(filename)
        .lines()
        .map(|x| {
            let (player1, player2): (Game, Game) = x
                .unwrap()
                .split_whitespace()
                .map(|x| Game::new(x.chars().next().unwrap()))
                .collect_tuple()
                .unwrap();
...
impl Game {
    ...
    pub fn get_move_to_play(opponent: &Game, result: &Game) -> Game {
